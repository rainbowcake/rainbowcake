...
The `Contexts` object that actually just contains `Dispatcher` instances has now been renamed `Dispatchers`. Client code shouldn't really use this object directly, so this shouldn't break anything.
